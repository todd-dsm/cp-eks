...
function createContext() {
    ...
    # get new kubeconfig
    pMsgS "  $myCluster"
    aws eks --region "$TF_VAR_region" update-kubeconfig --name "$myCluster"
    #echo "${myCluster##*/}" > "$myCluster"
    chmod 600 "$KUBECONFIG"
    # Change those obnoxious names
    renameCreds \
        "arn:aws:eks:${TF_VAR_region}:${TF_VAR_aws_acct_no}:cluster/${myCluster}" \
            "${myCluster%-*}"
}
...
set -x
while IFS=$'\t' read -r junk myCluster; do
    clusterContext="${myCluster%-*}"
    clusterKTXFile="${kubeConfDir}/${clusterContext}.ktx"
    # If the file does NOT exist
    if [[ ! -f "$clusterKTXFile" ]]; then
        createContext "$myCluster" "$clusterKTXFile"
    else
        # If the file DOES exist
        pMsgS "  $myCluster is already configured."
        #rm -f "$clusterKTXFile"
        #createContext
    fi
done < <(aws eks list-clusters --region="$TF_VAR_region" --output text)
#done < /var/tmp/clusters.out
set +x


###---
### Default to the Apps Cluster
###---
